...
(function () {
    ...
    function loadVar(key, def) {
        try { var fn = getVarFn('getVariables'); if (fn) { var v = fn({ type: 'character' }); if (v && key in v) return v[key]; } } catch (e) { console.warn('[CM] loadVar fail:', key, e); }
        return def;
    }
    function saveVar(key, val) {
        try { var fn = getVarFn('insertOrAssignVariables'); if (fn) { var o = {}; o[key] = val; fn(o, { type: 'character' }); return true; } } catch (e) { console.error('[CM] saveVar fail:', key, e); }
        return false;
    }
    function loadAll() {
        state.companyName = loadVar(V_COMPANY, '星耀传媒');
    ...
    function showToast(msg, isErr) {
        try { if (typeof toastr !== 'undefined' && toastr) { isErr ? toastr.error(msg) : toastr.success(msg); return; } } catch (e) {}
        console.log('[CM]' + (isErr ? ' [ERR] ' : ' ') + msg);
    }
    ...
    function sendToChat(text) {
        try {
            var ta = document.getElementById('send_textarea') || document.querySelector('#send_textarea') || document.querySelector('textarea[placeholder*="发送"]') || document.querySelector('textarea[id*="send"]');
            if (ta) {
                var proto = ta.tagName === 'TEXTAREA' ? window.HTMLTextAreaElement : window.HTMLInputElement;
                ...
                ta.focus();
                setTimeout(function () { var sendBtn = document.getElementById('send_but') || document.querySelector('#send_but') || document.querySelector('button[id*="send"]'); if (sendBtn) { try { sendBtn.click(); } catch (e) {} } }, 200);
                return true;
            }
    ...
    function createFab() {
        ...
        document.addEventListener('mousemove', function (e) { if (!isDragging) return; var dx = e.clientX - startX, dy = e.clientY - startY; if (Math.abs(dx) > 3 || Math.abs(dy) > 3) moved = true; fab.style.left = (origX + dx) + 'px'; fab.style.top = (origY + dy) + 'px'; fab.style.right = 'auto'; fab.style.bottom = 'auto'; });
        document.addEventListener('mouseup', function () { if (!isDragging) return; isDragging = false; fab.classList.remove('cm-dragging'); if (!moved) togglePanel(); });
    ...
    function openWorkForm(editId) {
        ...
        modal.querySelector('#wf-save').addEventListener('click', function () {
            ...
            sendToChat('【公司新增作品】分类：' + getCatLabel(cat) + '\n标题：' + title + '\n描述：' + desc);
